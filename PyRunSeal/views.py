...
def  BFV_Encrypt(request):  # x是要加密的明文,应该是int型整数，public_key是公钥，返回x对应的密文类
    if request.method == "POST":
        ...
        pub.close()
    encoder = IntegerEncoder(BFV_context)
    x_plain = Plaintext(encoder.encode(int(x)))
...
def BFV_Decrypt(request):  # x_encrypted是明文x对应的密文类，secret_key是密钥，返回x_encrypted对应的明文
    ...
    return JsonResponse({
        "x_decrypted": encoder.decode_int64(x_decrypted)
       # "x_decrypted":encoder.decode_int32(x_decrypted)
    })
...
def BFV_mul(request):  # x_encrypted,y_encrypted是明文x，y对应的密文类，返回x*y对应的密文类
    ...
    evaluator.multiply_many(lis_enc,relin_keys, mul_encrypted)
...
def BFV_squr(request):  # x_encrypted,y_encrypted是明文x，y对应的密文类，返回x*y对应的密文类
    if request.method == "POST":
        cip = open("cip","wb")
        cip.write(bytes.fromhex(request.POST.get("x_encrypted")))
        x_encrypted = Ciphertext()
    ...
    cipher.close()
    return JsonResponse({
        "squr_encrypted": squr_encrypted
    })



def BFV_sub(request):  # x_encrypted,y_encrypted是明文x，y对应的密文类，返回x-y对应的密文类
    if request.method == "POST":
        cip = open("cip","wb")
        cip.write(bytes.fromhex(request.POST.get("x_encrypted")))
        x_encrypted = Ciphertext()
    ...
    cipher = open("cipher","rb")
    sub_encrypted=''.join(['%02X' %x  for x in cipher.read()])
    cipher.close()
    return JsonResponse({
...
def CKKS_Decrypt(request):  # x_encrypted是明文x对应的密文类，secret_key是密钥，返回x_encrypted对应的明文
    if request.method == "POST":
        ...
        sec = open("sec","wb")
        sec.write(bytes.fromhex(request.POST.get("secret_key")))
        secret_key = KeyGenerator(CKKS_context).secret_key()
...
def CKKS_sub(request):  # x_encrypted,y_encrypted是明文x，y对应的密文类，返回x-y对应的密文类
    if request.method == "POST":
        cip = open("cip","wb")
        cip.write(bytes.fromhex(request.POST.get("x_encrypted")))
        x_encrypted = Ciphertext()
